 ...
 */
public abstract class BaseWebViewActivity<V extends MvpBaseInterface, P extends MvpBasePresenter<V>> extends BaseActivity<V, P> {
    ...
    protected void initView(Bundle savedInstanceState) {
        if (isNeedToolbar()) {
            initToolBar(false, "测试WebViewActivity");
        }
        //初始化WebView及相关控件
        initWebView();
    }
    ...
    private void initWebView() {
        ...
        webViewContentLayout = findViewById(R.id.activity_base_webView_content_layout);
        webViewProgress = findViewById(R.id.activity_base_webView_progress);
        webViewErrorLayout = findViewById(R.id.activity_base_webView_error_layout);
...
//            webSettings.setMixedContentMode(WebSettings.MIXED_CONTENT_ALWAYS_ALLOW);
    ...
    private class MeWebViewClient extends WebViewClient {
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            ...
            if (NetworkUtils.isConnectedAndAvailable(BaseWebViewActivity.this)) {        //网络连接并可用
        ...
        if (requestCode == PictureConfig.CHOOSE_REQUEST) {
            if (resultCode == RESULT_OK) {
                ...
                if (selectList != null && selectList.size() > 0){
                    ...
                    //将图片绝对路径转换为媒体Uri路径
                    Uri sourceUri = UriUtils.getMediaUriFromPath(BaseWebViewActivity.this, selectList.get(0).getPath());
                    if (sourceUri != null) {
